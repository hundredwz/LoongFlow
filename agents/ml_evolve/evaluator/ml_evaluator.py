...
class MLEvaluator(LoongFlowEvaluator):
    ...
    def _run_evaluate_target(evaluator_file_path: str, llm_file_path: str):
        ...
        try:
            # 1. Dynamically load the LLM code module to extract ML_EVOLVE_RESULT
            ...
            # Check for ML_EVOLVE_RESULT variable
            if not hasattr(llm_module, "ML_EVOLVE_RESULT"):
                raise AttributeError("Invalid llm_code, ML_EVOLVE_RESULT not found")

            ml_evolve_result = llm_module.ML_EVOLVE_RESULT

            if not isinstance(ml_evolve_result, dict):
                raise TypeError("ML_EVOLVE_RESULT must be a dict")

            logger.debug(f"[Child PID:{pid}] ML_EVOLVE_RESULT loaded successfully.")
            ...
            result = mod.evaluate(
                ml_evolve_result.get("task_data_path"),
                ml_evolve_result.get("best_code_path"),
                ml_evolve_result.get("artifacts"),
            )
